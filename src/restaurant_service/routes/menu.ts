...
router.post("/add_item", jsonParser, async (req, res) => {
  ...
  if (restaurant_id && name && price) {
    ...
    return res
      .status(400)
      .send(
        "Did you forget to send any of the fields? (name, price, restaurant_id)"
      );
  }
});

router.put("/update_item", jsonParser, async (req, res) => {
  const id = req.body.id ? req.body.id : null;
  const name = req.body.name ? req.body.name : null;
  const price = req.body.price ? req.body.price : null;

  if (id) {
    const response = await update_item(id, name, price);
    return res.status(parseInt(response[0])).send(response[1]);
  } else {
...
router.delete("/delete_item", jsonParser, async (req, res) => {
  const id = typeof req.query.id === "string" ? parseInt(req.query.id) : null;
  if (id) {
    const response = await remove_item(id);
    return res.status(parseInt(response[0])).send(response[1]);
  } else {
